...
export function BookCall() {
  ...
  return (
    ...
    >
      ...
      {/* 5. Coverage */}
      <div>
        <label className={labelClass}>What kind of coverage do you need? <span className="text-accent">*</span></label>
      ...
      <div>
        ...
        />
        <p className="mt-1 text-right text-[12px] text-muted">{formData.notes.length}/500</p>
      </div>

      <button
        type="submit"
        disabled={isSubmitting}
        className="self-start flex items-center gap-2 rounded-full bg-accent px-7 py-3 text-[15px] font-semibold text-white disabled:opacity-70"
      >
        {isSubmitting ? "Submitting…" : "Submit"}
        {!isSubmitting && <ArrowRight className="h-[16px] w-[16px]" />}
      </button>
    </form>
  );
}
