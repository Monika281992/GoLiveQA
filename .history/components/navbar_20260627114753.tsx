...
const engagementModels = [
  ...
  { title: "QA Outsourcing", desc: "End-to-end QA delivery, fully handled." },
];
    ...
    <header className="sticky top-0 z-50 border-b border-hairline bg-surface/80 backdrop-blur-md backdrop-saturate-150">
      <nav className="mx-auto flex max-w-container items-center justify-between gap-4 px-5 py-3.5 sm:px-8 lg:px-12">
        ...
        <div className="hidden items-center gap-7 md:flex">
          <a href="#about" className="text-[15px] font-medium text-body transition-colors hover:text-ink">
            About
          </a>

          {/* Services mega-menu dropdown */}
          <div className="relative" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
            <button className="flex items-center gap-1 text-[15px] font-medium text-body transition-colors hover:text-ink">
              Services
              <ChevronDown className={`h-4 w-4 transition-transform duration-200 ${dropdownOpen ? "rotate-180" : ""}`} />
            </button>

            {dropdownOpen && (
              <div className="absolute left-1/2 top-full mt-2 w-[760px] -translate-x-1/2 rounded-[18px] border border-hairline bg-white p-6 shadow-lift">
                <div className="grid grid-cols-4 gap-6">
                  {/* Col 1 */}
                  <div>
                    <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.08em] text-muted">Engagement Models</p>
                    <div className="flex flex-col gap-3">
                      {engagementModels.map((item) => (
                        <a key={item.title} href="#services" onClick={closeAll} className="group block">
                          <p className="text-[13.5px] font-semibold text-ink transition-colors group-hover:text-accent">{item.title}</p>
                          <p className="text-[12px] leading-snug text-muted">{item.desc}</p>
                        </a>
                      ))}
                    </div>
                  </div>
                  {/* Col 2 */}
                  <div>
                    <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.08em] text-muted">Focus Area</p>
                    <div className="flex flex-col gap-2">
                      {focusAreas.map((item) => (
                        <a key={item} href="#services" onClick={closeAll}
                          className="text-[13.5px] text-body transition-colors hover:text-accent">{item}</a>
                      ))}
                    </div>
                  </div>
                  {/* Col 3 */}
                  <div>
                    <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.08em] text-muted">Testing Coverage</p>
                    <div className="flex flex-col gap-2">
                  ...
                  {/* Col 4 */}
                  <div>
                    <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.08em] text-muted">Frameworks</p>
                    <div className="flex flex-col gap-2">
                      {frameworks.map((item) => (
                        <a key={item} href="#services" onClick={closeAll}
                          className="text-[13.5px] text-body transition-colors hover:text-accent">{item}</a>
          ...
          <a href="#process" className="text-[15px] font-medium text-body transition-colors hover:text-ink">Process</a>
          <a href="#contact" className="text-[15px] font-medium text-body transition-colors hover:text-ink">Contact</a>
        </div>
