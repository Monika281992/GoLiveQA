...
export function ProblemsWeSolve() {
  return (
    <section style={{ background: BG_SECTION, padding: "96px 32px" }}
      ...
      <style>{`
        .ql-disabled-row { position: relative; }
        .ql-tooltip-text {
          position: absolute;
          top: -40px;
          left: 50%;
        ...
        .ql-disabled-row:hover .ql-tooltip-text {
          opacity: 1;
          visibility: visible;
        }
        .ql-tooltip-text::after {
          content: '';
          position: absolute;
          bottom: -5px;
          left: 50%;
          ...
          {phases.map((phase, phaseIdx) => (
            <div key={phase.label}>
              ...
              {phase.items.map((item) => {
                ...
                return (
                  <div key={item.n}
                    className={isDisabled ? "ql-disabled-row" : ""}
                    style={{ display: "flex", gap: 28, alignItems: "flex-start", marginBottom: 16, opacity: isDisabled ? 0.45 : 1, filter: isDisabled ? "grayscale(1)" : "none", position: "relative" }}>
                    {isDisabled && (
                      <div className="ql-tooltip-text">Upgrade to Automation to include this</div>
                    )}
